...
- A company can submit one-time adjustments per employee per period (bonuses, extra deductions). [pending]
...
- Every state transition is recorded with a timestamp for audit purposes. [pending]
...
- If a payment ultimately fails, the reserved funds for that employee are returned to the company balance. [pending]
...
- The invoice includes: total gross salaries, total taxes withheld, and a flat platform fee of $29 per successfully paid employee.
- The invoice is immutable once generated — corrections require a new run or a credit note.
- Invoice status starts as `unpaid` and transitions to `paid` when the company settles it. [pending]
